...
postsSlothApiQuery = (f"access_token={accessToken}&include_deleted=true&deleted_at__gt={slothStart}&limit=1000&contentGroupId={contentGroupId}&property=stateWhenDeleted&property=id")
...
for slothedPostObject in slothedPostObjects:
    ...
    slothedPostStateWhenDeleted = slothedPostObject["stateWhenDeleted"]
    ...
    if restorePost.status_code == 200:
        if slothedPostStateWhenDeleted == "PUBLISHED":
            ...
            print (f"Restored post id {slothedPostId}, and left it in a draft state")
    else:
